...
if (isCartPage) {
  ...
  function displayCartItems() {
    ...
    // 檢查購物車是否為空
    var cartEmptyDiv = document.querySelector('.cart-empty');
    if (cartItemsContainer.children.length === 0) {
      cartEmptyDiv.classList.remove('hide'); // 移除 .hide 類別，顯示 .cart-empty 元素
    } else {
      cartEmptyDiv.classList.add('hide'); // 加上 .hide 類別，隱藏 .cart-empty 元素
    }
  }
